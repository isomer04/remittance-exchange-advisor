# Building a Remittance & Exchange Advisor Agent

A hands-on tutorial for building a conversational remittance advisor using Gemini function calling and Streamlit. The agent looks up exchange rates and transfer fees from a governed data sheet and calculates the exact amount a recipient will receive — the LLM never guesses numbers.

Built for fintech and remittance service audiences (product managers, engineers, compliance teams) who want to understand the agent pattern: **the LLM handles the conversation; the tools handle the data and math.**
...
| ----------------------------- | -------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `get_exchange_rate`           | Looks up the current exchange rate and transfer fees from a CSV data sheet | Exchange rates and fees are governed data — they change daily and must come from the platform's source of truth, not the LLM's training data |
| `calculate_remittance_amount` | Calculates the exact amount the recipient receives after fees and exchange | Remittance math must be deterministic and exact — even small rounding errors are unacceptable in a payments context                          |
